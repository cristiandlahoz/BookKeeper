...
Objetivo: mostrar opciones continuamente hasta que el usuario seleccione una
          o salga, a la vez que crea la animaci�n de moverse entre las opciones..
returno: Retorna la opcion seleccionada por el usuario de ser el caso. caso contrario
         retorna ESC.
*/
int opciones(int nmu)
{
   ...
   char respuesta[][COL_OPCIONS] = {
       "     SI      ",
       "     NO      "};

   char estructuras[][COL_OPCIONS] = {
       "LIBROS       ",
       "AUTORES      ",
       "USUARIOS     ",
       "PRESTAMOS    "};

   char LIBRO[][COL_OPCIONS] = {
       "ISBN         ",
       "TITULO       ",
       "AUTOR        ",
       "FECHA        ",
   ...
   char AUTOR[][COL_OPCIONS] = {
       "NOMBRES      ",
       "APELLIDOS    ",
       "FECHA        "};

   char USER[][COL_OPCIONS] = {
       "NOMBRES      ",
       "APELLIDOS    ",
       "TELEFONO     "};
...
Argumentos: Nodo_AT **cabeza. direcci�n de memoria de la cabeza del nodo de autores.
            NODO_AT **cola. direcci�n de memoria de la cabeza del nodo de autores.
...
Argumentos: FILE *pf. Puntero al file con los autores a leer.
            Nodo_AT **cabeza. direcci�n de memoria de la cabeza del nodo de los autores.
            NODO_AT **cola. direcci�n de memoria de la cabeza del nodo de los autores.
Objetivo: Leer todos los autores desde el archivo proporcionado y guardarlo en su nodo correspondiente.
*/
...
Argumentos: Nodo_LB **cabeza. direcci�n de memoria de la cabeza del nodo de libros.
            NODO_LB **cola. direcci�n de memoria de la cabeza del nodo de libros.
...
Argumentos: FILE *pf. Puntero al file con los libros a leer.
            Nodo_LB **cabeza. direcci�n de memoria de la cabeza del nodo de libros.
            NODO_LB **cola. direcci�n de memoria de la cabeza del nodo de los libros.
Objetivo: Leer todos los libros desde el archivo proporcionado y guardarlo en su nodo correspondiente
*/
...
Argumentos: Nodo_USR **cabeza. direcci�n de memoria de la cabeza del nodo de usuarios.
            NODO_UDR **cola. direcci�n de memoria de la cabeza del nodo de usuarios.
            USUARIO usr. cantidad total de usuarios.
Objetivo: isertar el usuario en la ultima posici�n del nodo.
*/
...
Argumentos: FILE *pf. Puntero al file con los usuarios a leer.
            Nodo_USR **cabeza. direcci�n de memoria de la cabeza del nodo de usuarios.
            NODO_UDR **cola. direcci�n de memoria de la cabeza del nodo de usuarios.
Objetivo: Leer todos los usuarios desde el archivo proporcionado y guardarlo en su nodo correspondiente
*/
...
Objetivo: mostrar todas las opciones del arreglo e identificar la fila seleccionada actualmente.
*/
...
Argumentos: NODO_AT *cabeza. Lista de autores a motrar. int x, y. posicion de columna y
            fila a usar en la terminal.
            int Fil. indice del autor seleccionado al momento de mostrar.
            int comienzo. indice desde donde se empezara a imprimir.
Objetivo: Mostrar los autores desde la posicion comienzo hasta mostrar, y resaltar
          el autor en que se encuentra el cursor en ese momento.
*/
void showautores_scroll(NODO_AT *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
   ScreenClear();
#define CARACTERES_NM_AP 10
   ...
   setcolor(YELLOW, BLACK);
   gotoxy(x, y);
   printf("%-*s%-*s%-*s%-*s", LENIDAUTOR, id, nb_m + 1, name,
          ap_m + 1, lname, STR_FECHA, fecha);
   ...
   for (int ind = comienzo, j = 0; ind < mostrar + comienzo; ind++, j++, nc = nc->sig)
   {
      ...
      printf("%-*s%-*s%-*s%2hu/%2hu/%-4hu", LENIDAUTOR, nc->at.idautor, nb_m + 1,
             nc->at.nombres, ap_m + 1, nc->at.apellidos, nc->at.fechanacimiento.dd,
             nc->at.fechanacimiento.mm, nc->at.fechanacimiento.yyy);
   }
   colordefault();
}
...
Argumentos: NODO_LB *cabeza. Lista de libros a motrar. int x, y. posicion de columna y
            fila a usar en la terminal.
            int Fil. indice del libro seleccionado al momento de mostrar.
            int comienzo. indice desde donde se empezara a imprimir.
Objetivo: Mostrar los libros desde la posicion comienzo hasta mostrar, y resaltar
          el libro en que se encuentra el cursor en ese momento.
*/
void showbooks_scroll(NODO_LB *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
...
#define CARACTERES_EDT 8
   ...
   setcolor(YELLOW, BLACK);
   gotoxy(x, y);
   printf("%-*s%-*s%-*s%-*s%-*s%-*s", LENISBN + 1, ISBN, ttl_m + 1, titulo, edt_m + 1, editora,
          LENIDAUTOR, id, STR_FECHA + 1, fecha, edn_m + 1, edicion);
   ...
   for (int ind = comienzo, j = 0; ind < mostrar + comienzo; ind++, j++, nc = nc->sig)
   {
      ...
      if (fil == ind)
         setcolor(BLACK, YELLOW);
      printf("%-*s%-*s%-*s%-*s%2hu/%2hu/%-4hu%*d", LENISBN + 1, nc->lb.ISBN, ttl_m + 1, nc->lb.titulo,
             edt_m + 1, nc->lb.editora, LENIDAUTOR, nc->lb.idautor,
             nc->lb.fechaedicion.dd, nc->lb.fechaedicion.mm, nc->lb.fechaedicion.yyy,
             edn_m + 1, nc->lb.edicion);
   }
   colordefault();
}
...
/**
Funci�n:capbooks.
Argumento: NODO_LB **cabeza. direcci�n de memoria del puntero hacia el primer nodo de la
           lista de libros.
           NODO_LB **cola. direcci�n de memoria del puntero hacia el ultimo nodo de la
           lista de libros
           NOTO_AT *cabeza. direcci�n de memoria del primer nodo de la lista de autores.
           int *cant_lb. puntero al total de libros capturados actualmente.
           int cant_at. cantidad total de libros.
Objetivo: capturar una estructura libro y guardarla en la lista de los libros
*/
void capbooks(NODO_LB **cabeza, NODO_LB **cola, NODO_AT *cabeza_at, int *cant_lb, int cant_at)
{
   LIBRO lb;
   ...
   do
   {
      ...
      do
      {
         chr = getch();
               ...
               chr != IZQUIERDA && chr != BKSP && chr != DERECHA && chr != ESP && chr != '-');
      if (chr == DERECHA)
      {
         if (ind < n - 1)
            ind++;
...
Objetivo: Mostrar los id de los autores contenido en NODO_AT y guardar el seleccionado en *idautora
          a la vez que se guarda el nombre y apellido del mismo.
return: 1 si se ha seleccionado un id y 0 en caso de que se haya cancelado.
*/
int capidautora_books(char *idautor, char *editora, NODO_AT *cabeza, int x, int y, int cant_at)
{
   int FIL = 0, comienzo = 0, scroll = SHOW_SCROLL;
   char s;
...
*/
void showid_scroll_lb(NODO_LB *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
   int size = 19;
...
FECHA evaluarfecha(char *str)
{
   ...
   for (int ind = 0; str[ind]; ind++)
      ...
      {
         char agnoo[] = {str[ind + 4], str[ind + 5], str[ind + 6], str[ind + 7], '\0'};
...
FECHA capfecha(char *str, int n, int x, int y)
{
   ...
   do
   {
      ...
      *(str + 5) = '/';
      ...
      {
         chr = getch();
      } while (!(isdigit(chr)) && chr != ENTER && chr != ESC && chr != IZQUIERDA &&
               chr != BKSP && chr != DERECHA && chr != ESP);
      ...
      if (chr == ENTER)
      {
         ...
         else if (cabeza)
         {
            *(str + ind) = '\0';
            ...
            {
               gotoxy(x, y + 3);
               printf("ERROR!!! ESTE ISBN YA SE ENCUENTRA CAPTURADO, INTENTE NUEVAMENTE\n");
               Sleep(1000);
               gotoxy(x, y + 3);
               delline();
...
/**
Funci�n: capautores.
Argumento: NODO_AT **cabeza. direcci�n de memoria del nodo que apunta a la primera posici�n.
           NODO_AT **cola. direcci�n de memoria del nodo de autores que apunta a la ultima posici�n.
           int *cant_at. direcci�n de memoria de la variable que contiene la cantidad de autores total.
Objetivo: capturar un autor y guardarlo en su lista.
*/
...
Objetivo: obtener el id correspondiente teniendo en cuenta el anterior y sumandole uno a este.
          en caso de ser el primero se llenara de ceros, y se le sumara 1 a la ultima posici�n.
*/
...
/**
Funci�n: capusuarios.
Argumento: NODO_USR **cabeza. direcci�n de memoria del nodo que apunta a la primera posici�n.
           NODO_USR **cola. direcci�n de memoria del nodo de usuarios que apunta a la ultima posici�n.
           int *cant_usr. direcci�n de memoria de la variable que contiene la cantidad de usuarios total.
Objetivo: capturar un usuario y guardarlo en su lista.
*/
...
Argumentos: NODO_USR *cabeza. Lista de usuarios a motrar. int x, y. posicion de columna y
            fila a usar en la terminal.
            int Fil. indice del libro seleccionado al momento de mostrar.
            int comienzo. indice desde donde se empezara a imprimir.
            int mostrar. cantidad de datos a mostrar.
Objetivo: Mostrar los usuarios desde la posicion comienzo hasta mostrar, y resaltar
          el usuario en que se encuentra el cursor en ese momento.
*/
void showusuarios_scroll(NODO_USR *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
...
#define CARACTERES_AP 10
   ...
   setcolor(YELLOW, BLACK);
   gotoxy(x, y);
   printf("%-*s%-*s%-*s%-*s", LENIDDOC, IDDOCUMENTO, nm_m + 1, nombres, apl_m + 1, apellidos,
          LENTELEFONO, telefono);
   ...
   for (int ind = comienzo, j = 0; ind < mostrar + comienzo; ind++, j++, nc = nc->sig)
   {
      ...
      if (fil == ind)
         setcolor(BLACK, YELLOW);
      printf("%-*s%-*s%-*s%-*s", LENIDDOC, nc->usr.iddocumento, nm_m + 1, nc->usr.nombres,
             apl_m + 1, nc->usr.apellidos, LENTELEFONO, nc->usr.telefono);
   }
   colordefault();
}
...
Argumento: NODO_LB *cabeza. direcci�n de memoria del puntero hacia el primer nodo de la
           lista de libros.
           NODO_AT *cabeza_at. direcci�n de memoria del primer nodo de la lista de autores.
           int cant_lb. cantidad total de libros.
           int cant_at. cantidad total de autores.
Objetivo: capturar una estructura libro y guardarla en la lista de los libros
*/
...
Objetivo: mostrar todas los id del arreglo e identificar el id seleccionado.
*/
void showid_scroll_at(NODO_AT *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
   int size = 17;
...
/**
Funci�n:MODautor.
Argumento: NODO_AT *cabeza. direcci�n de memoria del puntero hacia el primer nodo de la
           lista de autores.
           int cant_at. cantidad total de autores.
Objetivo: capturar una estructura libro y guardarla en la lista de los autores
*/
...
Objetivo: Mostrar los id de los autores contenido en NODO_AT y retornar el seleccionado.
return: 1 si se ha seleccionado un id y 0 en caso de que se haya cancelado.
*/
...
Objetivo: imprimir una fila de caracteres con los colores y tama�o correspondientes.
*/
...
/**
Funci�n:MODusuario.
Argumento: NODO_USR *cabeza. direcci�n de memoria del puntero hacia el primer nodo de la
           lista de usuarios.
           int cant_usr. cantidad total de usuarios.
...
Objetivo: Mostrar los id de los usuarios contenido en NODO_USR y retornar el seleccionado.
return: 1 si se ha seleccionado un id y 0 en caso de que se haya cancelado.
*/
...
Objetivo: mostrar todas los id del arreglo e identificar el id seleccionado.
*/
void showid_scroll_usr(NODO_USR *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
...
Argumentos: NODO_PRT **cabeza. direcci�n de memoria de la cabeza del nodo de prestamos.
            NODO_PRT **cola. direcci�n de memoria de la cabeza del nodo de prestamos.
            LIBRO lb. cantidad total de prestamos.
Objetivo: isertar el libro en la ultima posici�n del nodo.
*/
...
Argumentos: FILE *pf. Puntero al file con los prestamos a leer.
            NODO_PRT **cabeza. direcci�n de memoria de la cabeza del nodo de prestamos.
            NODO_PRT **cola. direcci�n de memoria de la cabeza del nodo de los prestamos.
Objetivo: Leer todos los prestamos desde el archivo proporcionado y guardarlo en su nodo correspondiente
*/
...
/**Funci�n: prestar.
Argumento: NODO_PRT **cabeza. doble puntero del primer nodo de la lista de prestamos.
           NODO_PRT **cola. doble puntero al ultimo nodo de la lista de prestamos.
           NODO_LB *cabeza_lb. puntero al primer nodo de la lista de libros.
...
Objetivo: prestar el libro seleccionado al usuario seleccionado e identificar si es posible o no.
*/
             ...
             NODO_USR *cabeza_usr, int *cant_prt, int cant_lb, int cant_usr)
{
   ...
   int i = 0;
   for (usuarios = cabeza_usr; i < showid_cursor_usr(cabeza_usr, 2, 4, cant_usr); usuarios = usuarios->sig)
      ;
   for (books = cabeza_lb, i = 0; i < showid_cursor(cabeza_lb, 2, 4, cant_lb); books = books->sig)
      ;
...
Argumentos: NODO_USR *cabeza. Lista de prestamos a motrar. int x, y. posicion de columna y
            fila a usar en la terminal.
            int Fil. indice del prestamo seleccionado al momento de mostrar.
            int comienzo. indice desde donde se empezara a imprimir.
            int mostrar. cantidad de datos a mostrar.
Objetivo: Mostrar los prestamos desde la posicion comienzo hasta mostrar, y resaltar
          el prestamo en que se encuentra el cursor en ese momento.
*/
void showusuarios_scroll_prt(NODO_PRT *cabeza, int x, int y, int fil, int comienzo, int mostrar)
{
...
#define CARACTERES_15 15
   ...
   setcolor(YELLOW, BLACK);
   gotoxy(x, y);
   printf("%-*s%-*s%-*s%-*s%-*s%-*s", LENIDDOC, IDDOCUMENTO, LENISBN + 1, isbn, CARACTERES_FC + 1, fprestamo,
          CARACTERES_FD, fdevolucion, CARACTERES_15, diasprestamo, CARACTERES_15, recargodiario);
   ...
   for (int ind = comienzo, j = 0; ind < mostrar + comienzo; ind++, j++, nc = nc->sig)
   {
      ...
      if (fil == ind)
         setcolor(BLACK, YELLOW);
      printf("%-*s%-*s%2hu/%2hu/%-4hu %2hu/%2hu/%-4hu%-*d%-*s", LENIDDOC, nc->prt.iddocumento, LENISBN, nc->prt.ISBN,
             nc->prt.fecprestamo.dd, nc->prt.fecprestamo.mm, nc->prt.fecprestamo.yyy, nc->prt.fecdevolucion.dd,
             nc->prt.fecdevolucion.mm, nc->prt.fecdevolucion.yyy, CARACTERES_15, nc->prt.diasprestamo,
