...
void clrscr() {
  ...
  FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                             __FOREGROUND + (__BACKGROUND << 4), 2000,
                             (COORD){0, 0}, &written);
  FillConsoleOutputCharacter(GetStdHandle(STD_OUTPUT_HANDLE), ' ', 2000,
                             (COORD){0, 0}, &written);
  gotoxy(1, 1);
}
...
void puttext(int left, int top, int right, int bottom, char *str) {
  ...
  for (i = n = 0; i <= bottom - top; i++)
    for (j = 0; j <= right - left && str[n] != 0; j++) {
      ...
      buffer[i][j].Attributes =
          FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED;
      n++;
    }

  WriteConsoleOutput(GetStdHandle(STD_OUTPUT_HANDLE), (CHAR_INFO *)buffer,
                     (COORD){80, 25}, (COORD){0, 0}, &r);
}
